...
export default function AddDetails() {
  ...
  const [characteristics, setCharacteristics] =
    useState<IDogCharacteristics>(dogCharacteristics);
  const [behavs, setBehavs] = useState<BehaviorsTypes[]>(
    dogCharacteristics.behaviors
  );
  ...
  return (
    <View style={{ padding: 7, backgroundColor: "#ffffff", borderRadius: 5 }}>
      ...
      <ScrollView
        style={{ backgroundColor: "#e8e9ed", borderRadius: 5, padding: 10 }}
        ...
        <KeyboardAvoidingView>
          <View style={styles.row}>
            <View style={{ width: "62%", marginRight: "5%" }}>
              <Text
                style={{ fontSize: 15, marginLeft: "1%", color: "#006aff" }}
              >
                Name
              </Text>
              <TextInput
                editable
                defaultValue={characteristics.name}
                placeholder="Name"
                style={styles.input}
                onChangeText={(value) =>
            ...
            <View style={{ width: "20%" }}>
              ...
              <NumericInput
                ...
                onChange={(value) =>
                  handleChange({ name: "age", value: value })
                }
                minValue={0}
                maxValue={100}
                step={1}
          ...
          <DropDownInput
            ...
            onValueChange={(el: dogEnums.BreedTypes) =>
              handleChange({ name: "breed", value: el })
            }
          />
          ...
          <DropDownInput
            ...
            onValueChange={(el: dogEnums.ColorTypes) => {
              handleChange({ name: "color", value: el });
            }}
          />
          ...
          <DropDownInput
            ...
            onValueChange={(el: dogEnums.SizeTypes) => {
              handleChange({ name: "size", value: el });
            }}
          />

          <DropDownInput
            data={dogArrays.HairTypes}
            placeholder="Hair"
            value={characteristics.hairLength}
            onValueChange={(el: dogEnums.HairTypes) => {
              handleChange({ name: "hairLength", value: el });
            }}
          />
          {/* <DropDownInput data={dogEnums.HeightTypes} placeholder="Height" value={height}
            onValueChange={(el: any) => { handleHeightChange(el) }} /> */}
          <DropDownInput
            data={dogArrays.TailTypes}
            placeholder="Tail"
            value={characteristics.tailLength}
            onValueChange={(el: dogEnums.TailTypes) => {
              handleChange({ name: "tailLength", value: el });
            }}
          />

          <DropDownInput
            data={dogArrays.EarsTypes}
            placeholder="Ears"
            value={characteristics.earsType}
            onValueChange={(el: dogEnums.EarsTypes) => {
              handleChange({ name: "earsType", value: el });
            }}
          />

          <DropDownInput
            data={dogArrays.SpecialMarkTypes}
            placeholder="Special marks"
            value={characteristics.specialMark}
            onValueChange={(el: dogEnums.SpecialMarkTypes) => {
              handleChange({ name: "specialMark", value: el });
            }}
          />
          ...
          <View style={{ flex: 1, flexDirection: "row", flexWrap: "wrap" }}>
            {dogArrays.BehaviorsTypes.map(
              (item: { label: string; value: string }) => {
                return (
                  <Bubble
                    key={item.label}
                    isSelected={behavs.includes(item.value)}
                    item={item.value}
                    placeholder={item.value}
                    onPress={() => handleBehaviourChange(item)}
                  ></Bubble>
                );
                //
              }
