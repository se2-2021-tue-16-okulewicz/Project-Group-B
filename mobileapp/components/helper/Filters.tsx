...
import { filterTypes, IFilterSort, initFilterProps, initTypeOfDogFilter, ISort, sortDirectionTypes, sortTypes, TypeOfDogFilter } from "./filtersInterface";
...
export default function Filters(props: any) {
  ...
  const [filters, setFilters] =
    useState<IFilterSort>(dogFilters);

  const [filter,setFilter] = useState<filterTypes | undefined>(dogFilters.filter);
  const [sort,setSort] = useState<ISort>(parseSort(filters.sort))
  const [dogType,setDogType] = useState<TypeOfDogFilter>(parseDogType(filters.filter?.isFound));
  ...
  function handleDogTypeChange({ name, value }: any) {
    ...
    let tmpDogType;
    if((newDogType.found && newDogType.lost) || (!newDogType.found && !newDogType.lost)){
  ...
  return (
    ...
    keyboardShouldPersistTaps="handled">
      <Text style={{fontSize:17,fontWeight:"bold", marginLeft:"1%", color:"grey"}}>SORT</Text>
      <DropDownInput
            data={sortTypes}
            placeholder="Sort by"
            value={sort.type}
...
<DropDownInput
            ...
            filter={true}
          />
      <Text style={{fontSize:17,fontWeight:"bold", marginLeft:"1%", color:"grey"}}>FILTER</Text>
          ...
          <View style={{flexDirection:"row"}}>
            <CheckBox checked={dogType.lost} onPress={() => handleDogTypeChange({ name: "lost", value: !dogType.lost })}></CheckBox>
            <Text style={{ fontSize: 15, marginLeft: "1%", color: "#006aff", alignSelf:"center" }}>Lost dogs</Text>
          </View>

          <View style={{flexDirection:"row"}}>
            <CheckBox checked={dogType.found} onPress={() => handleDogTypeChange({ name: "found", value: !dogType.found })}></CheckBox>
            <Text style={{ fontSize: 15, marginLeft: "1%", color: "#006aff", alignSelf:"center" }}>Found dogs</Text>
          </View>
