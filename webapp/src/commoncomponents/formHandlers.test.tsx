 ...
 */
 ...
 import { act, createRenderer, renderIntoDocument, Simulate } from "react-dom/test-utils";
...
import { DateFormControl, InputFormControl, SelectFormControl } from "./formHandlers";
...
import { OrderTypes } from "../dog/dogEnums";
 ...
 it("Rendered input form", () => {
   ...
   expect(container?.getElementsByClassName("input").length).toEqual(
     1
   );
 ...
 it("Rendered select form", async () => {
    act(() => {
      render(
        <SelectFormControl value={OrderTypes.DESC} class="input" label="label" name="test_id" options={OrderTypes}/>,
        container
      );
    });
    const values = {
    ...
    await act( async()=> {component?.dispatchEvent(new InputEvent("change", {data: OrderTypes.ASC}))});
  ...
  it("Rendered date form", () => {
    act(() => {
      render(
        ...
        <DateFormControl value={new Date()} class="input" label="label" name="test_id"/></MuiPickersUtilsProvider>,
        container
      );
    });
    ///check if form control was rendered
    ...
    expect(container?.getElementsByClassName("test_id").item.name).toEqual("item");
