  ...
  return (
    <FormControl variant="outlined" className={props.class}>
      <TextField
        ...
        label={
          (props.name as string).charAt(0).toUpperCase() + props.name.slice(1)
        }
    ...
    <MuiPickersUtilsProvider utils={DateFnsUtils}>
      <FormControl variant="outlined" className={props.class} margin="dense">
        <DatePicker
          ...
          clearable={true}
          onTouchCancel={(date: any) =>
            updateForm(new Date(1900, 1, 1, 0, 0, 0, 0), props.name)
          }
          onAbort={(date: any) =>
            updateForm(new Date(1900, 1, 1, 0, 0, 0, 0), props.name)
          }
...
export const SelectFormControl = (props: any) => {
  ...
  return (
    <FormControl variant="outlined" className={props.class} margin="dense">
      <InputLabel htmlFor={(props.name as string) + "-label"}>
        {(props.name as string).charAt(0).toUpperCase() + props.name.slice(1)}
      </InputLabel>
      <Select
        className={props.name}
