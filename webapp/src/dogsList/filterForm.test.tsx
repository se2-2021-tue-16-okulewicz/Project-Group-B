...
it("Rendered list of dogs", async () => {
  const props ={
    ...
    filters: {page:config.defaultFilters.page, size:config.defaultFilters.size}
  }
  act(() => {
    render(
  ...
  const input = renderIntoDocument(<InputFormControl value="test" class="input" label="label" name="test_id"/>);
  const inputContainer = renderIntoDocument(<FilterForm {...props}/>);
  await act( async()=> {container?.dispatchEvent(new InputEvent("change", {data: "0"}))});
    expect(props.filters.page).toEqual(0);
  expect(
    container?.classList.length
